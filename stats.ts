...
function getAboveBelow(nums : number[], value : number) : [number, number] {
    let aboveCount = 0;
    let belowCount = 0;
    for (const n of nums){
        if (n < value)
...
basicStatsAnalyzeButton.addEventListener("click", function () {
    ...
    (document.querySelector("#aboveBelow") as HTMLElement).textContent = `${getAboveBelow(numbers, mean).join(" & ")}`;
